...
//*/
	...
	function GetRedirectURL(){
		$params = array();
		$params['return_to'] = urlencode($this->URLs['approved']);
		$params['mode'] = 'checkid_setup';
		$params['identity'] = urlencode($this->openid_url_identity);
		$params['trust_root'] = urlencode($this->URLs['trust_root']);
		
		if (isset($this->fields['required'])
		  && (count($this->fields['required']) > 0)) {
			$params['sreg.required'] = implode(',',$this->fields['required']);
		}
		if (isset($this->fields['optional'])
		  && (count($this->fields['optional']) > 0)) {
			$params['sreg.optional'] = implode(',',$this->fields['optional']);
		}

 ...
 *
 * Функция явл. хорошим учебным пособием по изучению регулярных выражений и... русского мата! =)
 *
 ...
 *
 * TODO: добавить цифровую подделку с нулём под букву O
 *
 ...
 * @param    string    $replace       строка, на которую заменять матный фрагмент, например: '[ой]' ($replace д.б. в кодировке $charset)
 ...
 * @return   mixed(false/string/int)  Если $replace === NULL, то возвращает FALSE, если мат не обнаружен, иначе фрагмент текста с матерным словом.
 ...
 *                                      * PREG_BAD_UTF8_ERROR
 * @dependencies  strip_tags_smart(), UTF8::html_entity_decode(), UTF8::convert_case()
 * @since    2005
 ...
 * @version  3.2.4
 */
function censure($s, $delta = 3, $continue = "\xe2\x80\xa6", $is_html = true, $replace = null, $charset = 'UTF-8')
{
    ...
    $s       = _censure_to_utf8($s,       $charset);
    $replace = _censure_to_utf8($replace, $charset);
    ...
    if ($is_html)
    {
        ...
        if (! function_exists('strip_tags_smart')) require_once dirname(__FILE__) . '/strip_tags_smart.php'; #оптимизация скорости require_once
    ...
    if (strtoupper(substr($charset, 0, 3)) === 'UTF')  #UTF-8, UTF-16, UTF-32
    {
        /*
          ...
          Ещё пример -- знак ударения "\xcc\x81" (U+0301 «combining acute accent»).
        */
        #$s = preg_replace('/\p{M}++/suSX', '');  #alternative with /u flag
        $s = preg_replace('/(?: \xcc[\x80-\xb9]|\xcd[\x80-\xaf]  #UNICODE range: U+0300 - U+036F (for letters)
                           ...
                           /sxSX', '', $s);
        $s = str_replace("\xc2\xad", '', $s);  #вырезаем "мягкий" перенос строки (&shy;)
    }
    ...
    if ($replace === null || version_compare(PHP_VERSION, '5.2.0', '<'))
    {
        ...
        if ($result && $replace === null)
        {
            ...
            return _censure_from_utf8($fragment, $charset);
        }
        return false;
    }
    ...
    if ($result > 0)
    {
        ...
        foreach ($m[0] as $w)
        {
            $re_w = '~' . preg_replace_callback('~(?:/\\\\|[^\x20])~suSX', '_censure_make_regexp_callback', $w) . '~sxuiSX';
            $ss = preg_replace($re_w, $replace, $ss);
            #d($re_w);
        }
        while ($ss !== $s) $ss = censure($s = $ss, $delta, $continue, $is_html, $replace, 'UTF-8');
    }
    return _censure_from_utf8($ss, $charset);
}

#encodes string to UTF-8, if necessary
function _censure_to_utf8(/*string*/ $s, /*string*/ $charset)
{
    if (strtoupper($charset) === 'UTF-8') return $s;
    if (function_exists('iconv')) return iconv($charset, 'UTF-8//IGNORE//TRANSLIT', $s);
    return mb_convert_encoding($s, 'UTF-8', $charset);
}

#encodes string from UTF-8, if necessary
function _censure_from_utf8(/*string*/ $s, /*string*/ $charset)
{
    if (strtoupper($charset) === 'UTF-8') return $s;
    if (function_exists('iconv')) return iconv('UTF-8', $charset . '//IGNORE//TRANSLIT', $s);
    return mb_convert_encoding($s, $charset, 'UTF-8');
}

function _censure_make_regexp_callback(array &$m)
{
    #$re_holes = '[\x00-\x20\-_\*\~\.\'"\^=`:]';

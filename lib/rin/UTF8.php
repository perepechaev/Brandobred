...
class UTF8
{
    ...
    public static function convert_from($data, $charset = 'cp1251')
    {
        if (is_array($data))
        {
            ...
            foreach ($data as $k => &$v)
            {
                $k = self::convert_from($k, $charset);
                if ($k === false) return false;
                $d[$k] = self::convert_from($v, $charset);
        ...
        if (is_string($data))
        {
            if (function_exists('iconv')) return iconv($charset, 'UTF-8//IGNORE//TRANSLIT', $data);
            if (function_exists('mb_convert_encoding')) return mb_convert_encoding($data, 'UTF-8', $charset);
            if ($charset === 'cp1251' || $charset === 'cp1259') return self::convert_from_cp1259($data);
            if ($charset === 'UTF-16' || $charset === 'UCS-2') return self::convert_from_utf16($data);
            trigger_error('Charset "' . $charset . '" is not supported native, "iconv" or "mbstring" extension required', E_USER_WARNING);
            return false;
        }
     ...
     * @return  string
     */
    public static function convert_from_cp1259($s)
    {
     ...
     */
    public static function convert_from_utf16($s, $type = 'BE', $to_array = false)
    {
        static $types = array(
     ...
     * @param   bool                      $is_strict    строгая проверка диапазона ASCII? [^\x09\x0A\x0D\x20-\xFF]
     * @return  bool
     */
    public static function is_utf8($data, $is_strict = true)
    {
        if (is_array($data))
        {
            foreach ($data as $k => &$v) if (! self::is_utf8($v, $is_strict)) return false;
            return true;
        }
        elseif (is_string($data))
        {
            #the fastest variant:
            if (0 && function_exists('iconv')) #DEPRECATED
            {
            ...
            {
                if (! mb_check_encoding($data, 'UTF-8')) return false;
                if ($is_strict && preg_match('/[^\x09\x0A\x0D\x20-\xFF]/sSX', $data)) return false;
                return true;
            }

            $utf8_char_re = '(?>[\x09\x0A\x0D\x20-\x7E]           # ASCII
            ...
            */
            $result = $is_strict ? preg_replace('/' . $utf8_char_re . '*/sxSX', '', $data)
            ...
            #проверяем еще одним, самым медленным способом:
     ...
     */
    public static function check($str, $is_strict = true)
    {
     ...
     */
    public static function blocks_check($s, array $blocks)
    {
     ...
     */
    public static function chunk_split(/*string*/ $string, /*int*/ $length = null, /*string*/ $glue = null)
    {
    ...
    public static function str_limit($s, $maxlength = null, $continue = "\xe2\x80\xa6", &$is_cutted = null, $tail_min_length = 20) #"\xe2\x80\xa6" = "&hellip;"
    {
        ...
        if (self::strlen($tail) < $tail_min_length) return $s;
     ...
     */
    public static function str_split(/*string*/ $string, /*int*/ $length = null, $is_strict = true)
    {
        if (! is_string($string)) trigger_error('A string type expected in first parameter, ' . gettype($string) . ' given!', E_USER_ERROR);
    ...
    */
    ...
    {
        if (! is_array($a = self::str_split($string))) return false;
     ...
     */
    public static function substr_replace(/*string*/ $string,
                                 /*string*/ $replacement,
                                 /*int*/    $start,
                                 /*int*/    $length = null)
    {
     ...
     * @return   string
     */
    public static function unescape($s, $is_rawurlencode = false)
    {
        ...
        return preg_replace_callback('/%u(  [\da-fA-F]{4}+          #%uXXXX     only UCS-2
                                      ...
                                      /sxSX', array('self', $is_rawurlencode ? '_unescape_rawurlencode' : '_unescape'), $s);
    }
    ...
    {
        $codepoint = hexdec(trim($m[1], '{}'));
        return rawurlencode(self::chr($codepoint));
    }

    /**
     * Рекурсивный вариант self::unescape()
     *
     * @see      self::unescape()
     * @param    mixed(string/array) $data
     * @return   mixed
     */
    public static function unescape_recursive($data, $is_rawurlencode = false)
    {
        if (is_array($data))
        {
            $d = array();
            foreach ($data as $k => &$v) $d[self::unescape($k, $is_rawurlencode)] = self::unescape_recursive($v, $is_rawurlencode);
            return $d;
        }
        else return self::unescape($data, $is_rawurlencode);
    }
     ...
     * @return  int
     */
    function textarea_rows($s, $cols, $min_rows = 3, $max_rows = 32)
    {
        if (strlen($s) == 0) return $min_rows;
        $rows = 0;
